...
def typing(path):
    def check(path2):
        if path2 == path or path2 + "/" == path:
            return True
        else:
            return False

    if check("tos"):
        return render_template("typing_tos.html")
    elif check("rule"):
        return render_template("typing_rule.html")
    elif check("ranking"):
        return render_template("typing_ranking.html")
...
def ranking():
    if request.json == {}:
        f = open("./ranking.pic", "rb")
        ranking = pickle.load(f)
        f.close()
        return jsonify([None, ranking, False])
...
@app.route("/ranking-reset", methods=["GET"])
def ranking_reset():
    try:
        if request.args.get('pass', '') == "hk20080522":
            path = "./ranking.pic"
        ...
        print(e)
    return render_template("404.html", message="ページが見つかりませんでした")
